...
cd ../nginx
docker buildx build --platform linux/amd64 -t kubeshark/k6s-load-server:$VERSION . --push

# Build and push the k6 load test image
cd ../k6
docker buildx build --platform linux/amd64 -t kubeshark/k6s-load-test:$VERSION . --push
...
make build
docker buildx build --platform linux/amd64 -t kubeshark/go-http2-client:$VERSION . --push
...
make build
docker buildx build --platform linux/amd64 -t kubeshark/go-http2-server:$VERSION . --push